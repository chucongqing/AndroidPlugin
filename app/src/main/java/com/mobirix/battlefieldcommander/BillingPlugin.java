 ...
 */
package com.mobirix.battlefieldcommander;
...
import com.mobirix.battlefieldcommander.activity.BillingNativeActivity;
...
public class BillingPlugin {
	...
	public void initBilling(String publicKey, final String inappSkus, final String subsSkus) {
		...
		mHelper.startSetup(new IabHelper.OnIabSetupFinishedListener() {
			public void onIabSetupFinished(IabResult result) {
				if (!result.isSuccess()) {
					Log.i(TAG,"result is failed! msg = " + result.getMessage());
					mCallBack.initMessage(false);
					return;
				}
				Log.i(TAG,"onIabSetupFinished  Success! " + result.getMessage());
	...
	IabHelper.QueryInventoryFinishedListener mGotInventoryListener = new IabHelper.QueryInventoryFinishedListener() {
		public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
			...
			if (result.isFailure()) {
				Log.i(TAG,"QueryInventory is failed! + msg = " + result.getMessage());
				mCallBack.initMessage(false);
				return;
			}
			Log.i(TAG,"QueryInventory  Success! " + result.getMessage());
			// 情報を内部に保持する.
	...
	// 購入完了イベントの通知.
	//Notification of purchase completion event.
	IabHelper.OnIabPurchaseFinishedListener mPurchaseFinishedListener = new IabHelper.OnIabPurchaseFinishedListener() {
		public void onIabPurchaseFinished(IabResult result, Purchase purchase) {
			if (result.isFailure()) {
				...
				mCallBack.purchaseMessage( false,sku,"");
				initState();
				return;
			}
			...
			if (mPurchaseState.equals(PurchaseState.ConsumablePurchase)) {
				...
				//In the case of consumer items, we will consume the Google side.
				mHelper.consumeAsync(purchase, mConsumeFinishedListener);
			} else {
				// purcase success event.
				mCallBack.purchaseMessage( true,purchase.getOriginalJson(),purchase.getSignature());
	...
	IabHelper.OnConsumeFinishedListener mConsumeFinishedListener = new IabHelper.OnConsumeFinishedListener() {
		public void onConsumeFinished(Purchase purchase, IabResult result) {
			...
			if (result.isSuccess()) {
				...
				mCallBack.purchaseMessage( false,purchase.getOriginalJson(),purchase.getSignature());
			}
			initState();
		}
	};
